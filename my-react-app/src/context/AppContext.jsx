...
export const AppProvider = ({ children }) => {
  ...
  // ✨ LOCALSTORAGE CACHING FOR PRODUCTS (Slow internet par khali boxes rokne ke liye)
  const [products, setProducts] = useState(() => {
    const saved = localStorage.getItem("luxury_products");
    return saved ? JSON.parse(saved) : [];
  });
  const [loadingProducts, setLoadingProducts] = useState(() => {
    return !localStorage.getItem("luxury_products");
  });

  // ✨ LOCALSTORAGE CACHING FOR STORIES
  const [videoGalleryData, setVideoGalleryData] = useState(() => {
    const saved = localStorage.getItem("luxury_stories");
    return saved ? JSON.parse(saved) : [];
  });
  const [loadingStories, setLoadingStories] = useState(() => {
    return !localStorage.getItem("luxury_stories");
  });

  // ✨ LOCALSTORAGE CACHING FOR INSTAGRAM POSTS
  const [instagramPosts, setInstagramPosts] = useState(() => {
    const saved = localStorage.getItem("luxury_instagram");
    return saved ? JSON.parse(saved) : [];
  });
  const [loadingInstagramPosts, setLoadingInstagramPosts] = useState(() => {
    return !localStorage.getItem("luxury_instagram");
  });
  ...
  const fetchProducts = async (isInitial = false) => {
    try {
      ...
      localStorage.setItem("luxury_products", JSON.stringify(finalData));
    } catch (error) {
      console.error("Failed to load live products:", error.message);
    } finally {
      if (isInitial) setLoadingProducts(false);
  ...
  const fetchStories = async (isInitial = false) => {
    try {
      ...
      const data = Array.isArray(response.data) ? response.data : response.data.stories || [];
      setVideoGalleryData(data);
      localStorage.setItem("luxury_stories", JSON.stringify(data));
    } catch (error) {
      console.error("Failed to load live stories:", error.message);
    } finally {
      if (isInitial) setLoadingStories(false);
  ...
  const fetchInstagramPosts = async (isInitial = false) => {
    try {
      ...
      const data = Array.isArray(response.data) ? response.data : response.data.posts || [];
      setInstagramPosts(data);
      localStorage.setItem("luxury_instagram", JSON.stringify(data));
    } catch (error) {
      console.error("Failed to load live instagram posts:", error.message);
    } finally {
      if (isInitial) setLoadingInstagramPosts(false);
