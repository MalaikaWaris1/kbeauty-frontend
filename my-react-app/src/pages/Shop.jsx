...
const FALLBACK_PRODUCTS = [
  {
    ...
    name: "Moonlit Recovery Balm",
    category: "Moisturizers",
    price: 13344.00, // PKR equivalent example for fallback
    description: "Overnight barrier repair",
    image: "https://images.unsplash.com/photo-1601049676099-e7ed07d825b0?q=80&w=500&auto=format&fit=crop",
    tag: "NEW",
    discount: null,
          ...
          {!loadingProducts && (
            <div className="shop-products-grid">
              {filteredProducts.map((product) => {
                ...
                // Admin price is in PKR
                const productPricePKR = Number(product.price) || 0;
                const productOldPricePKR = product.originalPrice || product.oldPrice ? Number(product.originalPrice || product.oldPrice) : null;
                
                // Calculate USD equivalent from PKR price
                const productPriceUSD = exchangeRate > 0 ? productPricePKR / exchangeRate : 0;
                const productOldPriceUSD = productOldPricePKR && exchangeRate > 0 ? productOldPricePKR / exchangeRate : null;
                  ...
                  >
                    ...
                    <div className="product-card-details">
                      <div className="prod-title-row">
                        ...
                        <div className="prod-price-box">
                          {productOldPricePKR && (
                            <span className="old-price" style={{display: "flex", flexDirection: "column", alignItems: "flex-end"}}>
                              <span>${productOldPriceUSD.toFixed(2)}</span>
                              <span style={{fontSize: "0.75em", color: "#888"}}>PKR {productOldPricePKR.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                            </span>
                          )}
                          <span className="current-price" style={{display: "flex", flexDirection: "column", alignItems: "flex-end"}}>
                            <span>${productPriceUSD.toFixed(2)}</span>
                            <span style={{fontSize: "0.75em", color: "#888"}}>PKR {productPricePKR.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                          </span>
                        </div>
                      </div>
                      <p className="prod-short-desc">{product.description}</p>
